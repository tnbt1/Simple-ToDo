...
アプリケーションに初めてアクセスすると、テスト用アカウントが自動的に作成されます：
- メールアドレス: test@example.com
- パスワード: test123

新規アカウントを作成する場合は、サインアップページから登録してください。

※ テストユーザーは`/api/init`エンドポイントによって初回アクセス時に作成されます。
...
テストユーザー（test@example.com）は初回アクセス時に自動的に作成されます。
手動で作成する場合：
```bash
...
起動時にRedisの警告が表示される場合は、quick-start.shの実行時に最適化をスキップした可能性があります。
警告を解消するには：
```bash
